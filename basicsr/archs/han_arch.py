...
def window_reverse(windows, window_size, h, w):
    ...
    if b != 0:
        ...
        x = windows.view(1, h // window_size[0], w // window_size[1], window_size[0], window_size[1], -1)
        x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(1, h, w, -1)
    return x
...
class HSA(nn.Module):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.logit_scale1 = nn.Parameter(torch.log(10 * torch.ones((num_heads, 1, 1))), requires_grad=True)
    ...
    def forward(self, x, mask=None):
        ...
        attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1))  # b,n,n
        attc = (F.normalize(q, dim=-1).transpose(-2, -1)) @ F.normalize(k, dim=-1)  # b,c,c


        logit_scale = torch.clamp(self.logit_scale, max=torch.log(torch.tensor(1. / 0.01)).item()).exp()
        logit_scale1 = torch.clamp(self.logit_scale1, max=torch.log(torch.tensor(1. / 0.01)).item()).exp()
        attn = attn * logit_scale
        attc = attc * logit_scale1
        ...
        attc = self.softmax(attc)


        x = (attn @ v).transpose(1, 2).reshape(b_, n, c)

        x1 = (attc @ v.transpose(-2, -1)).transpose(-2, -1).reshape(b_, n, c)
        x = x + x1
...
class HSAB_Group(nn.Module):
    def __init__(self,
        ...
        self.atten = nn.Conv2d(dim, dim*2, 1)
        self.dwconv = nn.Conv2d(dim*2, dim*2, 3, 1, 1, groups=dim*2)
        self.endconv = nn.Conv2d(dim*2, dim, 1)
        self.gelu = nn.GELU()
        self.li = nn.Sequential(nn.Conv2d(dim, dim//10, 1),
                                nn.ReLU(True),
                                nn.Conv2d(dim//10, dim, 1))
    ...
    def forward(self, x, x_size):
        ...
        x = self.dwconv(self.conv(x)) * self.atten(x)
        x = self.gelu(x)
        x = self.endconv(x)
        x = self.sig(self.li(self.avg(x))) * x
...
class HAN(nn.Module):
    def __init__(self,
        ...
        self.conv_first = nn.Sequential(nn.Conv2d(in_channels=in_chans*4, out_channels=embed_dim, kernel_size=1),
        ...
        self.conv_after_body = nn.Sequential(nn.Conv2d(embed_dim, embed_dim, 1),
                                             nn.Conv2d(embed_dim, embed_dim, 3, 1, 1, groups=embed_dim))
